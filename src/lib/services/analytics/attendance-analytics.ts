...
function buildAttendanceItemsQuery(whereClause: SQL | undefined, limit: number, offset: number) {
  return db
    .select({
      ...
      address: atendimentos.endereco,
      bairro: atendimentos.bairro,
      attendant: atendimentos.atendente,
      indication: atendimentos.indicacao,
      phones: atendimentos.telefones,
      company: atendimentos.empresa,
      ...
      withinSla: atendimentos.dentroSla,
...
export async function getAttendancesAnalytics(filters: ExternalApiFilters) {
  ...
  const [[totals], grouped, byType, byTechnician, items] = await Promise.all([
    db
      .select({
        ...
        closed: sql<number>`cast(sum(case when ${atendimentos.finalizacaoAt} is not null then 1 else 0 end) as int)`,
        open: sql<number>`cast(sum(case when ${atendimentos.finalizacaoAt} is null then 1 else 0 end) as int)`,
        withinSla: sql<number>`cast(sum(case when ${atendimentos.dentroSla} = true then 1 else 0 end) as int)`,
        outsideSla: sql<number>`cast(sum(case when ${atendimentos.dentroSla} = false then 1 else 0 end) as int)`,
        withinSlaUtil: sql<number>`cast(sum(case when ${atendimentos.dentroSlaUtil} = true then 1 else 0 end) as int)`,
        outsideSlaUtil: sql<number>`cast(sum(case when ${atendimentos.dentroSlaUtil} = false then 1 else 0 end) as int)`,
      })
      .from(atendimentos)
      .where(whereClause),
  ...
  return {
    totals: {
      ...
      open: Number(totals?.open ?? 0),
      closed: Number(totals?.closed ?? 0),
      withinSla: Number(totals?.withinSla ?? 0),
      outsideSla: Number(totals?.outsideSla ?? 0),
      withinSlaUtil: Number(totals?.withinSlaUtil ?? 0),
      outsideSlaUtil: Number(totals?.outsideSlaUtil ?? 0),
    },
    grouped: grouped.map((row) => ({
      bucket: row.bucket,
      total: Number(row.total),
...
export async function getSlaAnalytics(filters: ExternalApiFilters) {
  ...
  return {
    totals: {
      ...
      withinSla: totals.withinSlaCorrido,
      outsideSla: Math.max(totals.concluded - totals.withinSlaCorrido, 0),
      ...
      avgSlaPercent: totals.concluded > 0 ? Number((totals.withinSlaCorrido / totals.concluded).toFixed(4)) : 0,
